/// reference: http://graphics.stanford.edu/~drussel/Argument_helper/index.html
/*
 *
 * Argument Helper
 *
...
namespace dsr {
    ...
    */
    ...
    class Argument_helper::BoolTarget: public Argument_target{
    public:
        bool &val;
        BoolTarget(const char *arg_descr,
                   const char *descr, bool &b): Argument_target(0, std::string(),
                                                                std::string(descr),
                                                                std::string(arg_descr)),
                                                val(b){}
        BoolTarget(char k, const char *lname,
                   const char *arg_descr,
                   const char *descr, bool &b): Argument_target(k, std::string(lname),
                                                                std::string(descr),
                                                                std::string(arg_descr)),
                                                val(b){}
        virtual bool process(int &argc, const char **&argv){
    ...
    class Argument_helper::UnsignedVectorTarget: public Argument_target{
    public:
        std::vector<unsigned> &val;

        UnsignedVectorTarget(char k, const char *lname, const char *arg_descr,
                             const char *descr, std::vector<unsigned> &b): Argument_target(k, lname, descr,
                                                                                           arg_descr),
                                                                           val(b){}

        UnsignedVectorTarget(const char *arg_descr,
                             const char *descr, std::vector<unsigned> &b): Argument_target(0, std::string(),
                                                                                           std::string(descr),
                                                                                           std::string(arg_descr)),val(b){}

        virtual bool process(int &argc, const char **&argv){
            while (argc >0 && argv[0][0] != '-'){
                val.push_back(atoi(argv[0]));
                --argc;
                ++argv;
            }
            return true;
    ...
    void Argument_helper::new_named_unsigned_vector(char key, const char *long_name,
                                                    const char *arg_description, const char *description,
                                                    std::vector<unsigned> &dest){
        Argument_target *t= new UnsignedVectorTarget(key, long_name, arg_description, description, dest);
        new_argument_target(t);
    };


    /**
     * @note it has to be at the tail of (not optional) param list, because we do not know when will it end.
     * @param arg_description
     * @param description
     * @param dest
     */
    void Argument_helper::new_unsigned_vector(const char *arg_description, const char *description,
                                              std::vector<unsigned> &dest){
        Argument_target *t= new UnsignedVectorTarget(arg_description, description, dest);
        unnamed_arguments_.push_back(t);
        all_arguments_.push_back(t);
    };
    ...
    void Argument_helper::new_named_bool(char key, const char *long_name,
                                         const char *arg_description, const char *description,
                                         bool &dest){
        Argument_target *t= new BoolTarget(key, long_name, arg_description, description, dest);
        new_argument_target(t);
    };



    void Argument_helper::new_bool(const char *arg_description, const char *description,
                                   bool &dest){
        Argument_target *t= new BoolTarget(arg_description, description, dest);
        unnamed_arguments_.push_back(t);
        all_arguments_.push_back(t);
    };
    ...
    void Argument_helper::write_usage(std::ostream &out) const {
        ...
        out << "Usage: " << name_  << " ";
        for (auto unnamed_argument : unnamed_arguments_){
            unnamed_argument->write_name(out);
            out << " ";
        }
        for (auto optional_unnamed_argument : optional_unnamed_arguments_){
            out << "[";
            optional_unnamed_argument->write_name(out);
            out << "] ";
        }
        ...
        out << "All arguments:\n";
        for (auto unnamed_argument : unnamed_arguments_){
            unnamed_argument->write_usage(out);
        }
        for (auto optional_unnamed_argument : optional_unnamed_arguments_){
            optional_unnamed_argument->write_usage(out);
        }

        out << extra_arguments_arg_descr_ << ": " << extra_arguments_descr_ << std::endl;
        for (const auto & short_name : short_names_){
            (short_name.second)->write_usage(out);
        }
    }
    ...
    void Argument_helper::write_values(std::ostream &out) const {
        for (auto unnamed_argument : unnamed_arguments_){
            out << unnamed_argument->description;
            out << ": ";
            unnamed_argument->write_value(out);
            out << std::endl;
        }
        for (auto optional_unnamed_argument : optional_unnamed_arguments_){
            out << optional_unnamed_argument->description;
            out << ": ";
            optional_unnamed_argument->write_value(out);
            out << std::endl;
        }
        if (extra_arguments_!=nullptr){
            for (const auto & extra_argument : *extra_arguments_){
                out << extra_argument << " ";
            }
        }

        for (const auto & short_name : short_names_){
            out << short_name.second->description;
            out << ": ";
            short_name.second->write_value(out);
            out << std::endl;
        }
    }

    Argument_helper::~Argument_helper(){
        for (auto & all_argument : all_arguments_){
            delete all_argument;
        }
    }
